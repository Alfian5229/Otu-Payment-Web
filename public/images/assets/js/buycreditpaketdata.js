app.controller('buycreditpaketdataCtrl', function($scope, $http) {
    ...
    if(isAllLoad == false){
        ...
        $.post("../buycreditpaketdata/getjsonprefixdata", function(data){
            prefix = data;
        });
        //get data json for paket data
        $.post("../buycreditpaketdata/getjsonPaketData", function(data){
            hasildata = data;
        });
        isAllLoad = true;
    }
        ...
        angular.forEach(entities, function(subscription, index) {
            ...
            if (position == index){
                if ($scope.prefixes[position].checked == true){
                    ...
                    $scope.nominal = "paket data " + $scope.prefixes[position].name;
                    toBottom();
                }
    ...
    $scope.tampilModalAlert = function(){
        $.post("../buycreditpaketdata/buypaketdata", { kodeitem: kodeItem, nomerhp: $scope.myValue} ,function(data){
    ...
    //convert provider to provider data
    function provToProvData(provider){
        if(provider == "Telkomsel"){
            providerHasil = "TSEL DATA";
        }
        if(provider == "XL"){
            providerHasil = "XL DATA";
        }
        if(provider == "Indosat"){
            providerHasil = "ISAT DATA";
        }
        if(provider == "Three"){
            providerHasil = "TRI DATA";
        }
        if(provider == "Smart"){
            providerHasil = "SMARTFREN DATA";
        }
        if(provider == "Axis"){
            providerHasil = "AXIS DATA";
        }
    }

    // function for check data paketdata with api
    function getDataPaketData(providerdata){
        $.post("../buycreditpaketdata/getdataPaketData", { providerpaketdata: providerdata} ,function(data){
            hasilSearch = data;
        });
    }
});